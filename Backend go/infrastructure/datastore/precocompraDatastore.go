...
func (d *PrecoCompraDatastore) GetValoresCompraVenda(ctx context.Context, saleId int) ([]*entity.DifValue, error) {
	query := `select vcc.id_valores_compra_venda,
 vcc.sale_id,
  vcc.valor_venda_produto,
   vcc.valor_compra_produto,
    vcc.product_id,
	( select si.barcode 
	 from sale_items si 
	 where si.sale_id = vcc.sale_id 
	 and si.product_id = vcc.product_id 
	 limit 1 
	) as barcode
	from valores_compra_venda vcc where vcc.sale_id = $1;`
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("Erro ao listar os dados da venda: %w", err)
		}
